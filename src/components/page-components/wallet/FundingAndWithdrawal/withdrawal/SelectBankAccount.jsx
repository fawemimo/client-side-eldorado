...
export default function SelectBankAccount({ onBankSelect }) {
	...
	return (
		<DropdownSelect
			dropdownWidth={"100%"}
			items={rootState.userBanks}
			renderItem={({ item, index, setOpen }) => (
				<button
					type="button"
					onClick={() => {
						setSelectedBank(`${item.account_name}-${item.account_number}`);
			...
			renderButton={({ open, setOpen }) => (
				<div className="flex flex-col gap-y-3">
					...
					<button
						...
						className="flex items-center bg-white justify-between gap-5 text-2xl font-medium border-[1.5px] border-[#E6E6E6] rounded-xl px-5 h-[5rem]">
						{selectedBank === "" ? (
