...
import { TransactionSuccessB } from "../../transactions/TransferConfirmationModal";
import useHelpers from "@/hooks/useHelpers";
import axios from "axios";

// 1. this is the endpoint for creating the invoice invoice/invoices
// 2.  for the client to view the account and make payments this is the endpoints http://127.0.0.1:8000/transaction/transfer/invoice-payments/?invoice-id=INV1751129716289ecda
// 3. for verifying the client payment  http://127.0.0.1:8000/transaction/transfer/verify-invoice-payment/?invoice-id=INV1751129716289ecda

// NOTE THAT 2 AND 3 ABOVE does not have request data

export default function InvoicePaymentScreen({ accountDetails }) {
	...
	const { data, error, isLoading, refetch, isFetching } = useQuery({
		...
		queryFn: async () => {
			const response = await axios.post(
				`https://backend-c33q.onrender.com/transaction/transfer/invoice-payments/?invoice-id=${invoiceId}`
			);
	...
	async function verifyPayment() {
		try {
			setVerifying(true);
			const response = await axios.post(
				`https://backend-c33q.onrender.com/transaction/transfer/verify-invoice-payment/?invoice-id=${invoiceId}`
			);
